...
// Actualizar cliente
async function actualizarCliente(idcliente: number) {
  const response = await fetch("http://localhost:4000/api/clientes", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function buscarClientes(query: string) {
  const response = await fetch(
    `http://localhost:4000/api/clientes/search?q=${encodeURIComponent(query)}`,
  );
  const clientes = await response.json();
...
// Obtener un cliente
async function obtenerCliente(idcliente: number) {
  const response = await fetch(
