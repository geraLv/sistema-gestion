...
const FECHA_MIGRACION = new Date('2025-02-15T00:00:00');
...
async function syncCuotas() {
        ...
        for (const solId of batchIds) {
            ...
            for (const dbCuota of myCuotas) {
                ...
                if (csvCuota.estado === 2 && dbCuota.estado === 0) {
                    ...
                    let esFalsoPagado = false;

                    if (!dbCuota.fecha) {
                        esFalsoPagado = true;
                    } else {
                        const fechaPagoDB = new Date(dbCuota.fecha);
                        if (fechaPagoDB < FECHA_MIGRACION) {
                            esFalsoPagado = true;
                        }
                    }
    ...
    console.log(`💎 Pagos reales recientes respetados: ${stats.pagosNuevosRespetados}`);
    console.log(`❌ Errores de DB durante guardado: ${stats.errores}`);
}

syncCuotas().catch(err => console.error(err));
